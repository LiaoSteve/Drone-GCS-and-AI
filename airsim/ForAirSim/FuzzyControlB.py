...
class FuzzyControl(object):
    def __init__(self):
        ...
        self.v_universe  = np.arange(-2, 2, 0.1)
        self.vz_universe = np.arange(-1.0, 1.5, 0.1)      # m/s, output
    ...
    def Membership(self):
        ...
        self.z_input['far']  = fuzz.trapmf(self.z_input.universe, [100, 300, 500, 500])
        #self.z_input['near'] = fuzz.zmf(self.z_input.universe, 180, 750)
        self.z_input['near']  = fuzz.trapmf(self.z_input.universe, [0, 0, 80, 200])
        ...
        self.vz['fast'] = fuzz.trapmf(self.vz.universe, [-0.3, 1.2, 1.5, 1.5])
        self.vz['slow'] = fuzz.trapmf(self.vz.universe, [-1.0, -1.0, -0.8, 1.0])
...
if __name__ == '__main__':
    ...
    fz = FuzzyControl()   
    fz.pre_fzprocess()
    vx, vy, vz, cost_t = fz.fzprocess(delta_x=220, delta_y=0, distance=300)    
