...
class DroneSystem(GCS, Cam, CamSocket, RealSense, YOLO):
    ...
    def mark_yolo_result(self):
        trash_num = cap_num = plastic_num = 0
        ...
        data['plastic_num'] = str(plastic_num)
    ...
    def yolo_thread_start(self):
        def yolo_job():            
            while 1:        
                try:                    
                    if not len(self.cam_Frame) or self.yolo_thread_isstop: 
                        break            
...
if __name__ =='__main__':        
    ...
    try:
        ...
        while 1:                      
            DS.goto(DS.target,groundspeed = 1)                              
