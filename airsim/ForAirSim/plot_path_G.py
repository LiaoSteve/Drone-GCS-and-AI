...
if __name__ == '__main__':
    ...
    ob = get_object_position_from_txt('record_position/object_G.txt')    
    trajectory = np.array(get_path_data_from_txt('record_position/G.txt'))
    wp = get_object_position_from_txt('record_position/wp_G.txt')
    ...
    for i in range(len(ob)):
        plot_opaque_cube(ax1,ob[i][0]-l/2, ob[i][1]-l/2, ob[i][2], l, l, l)  # 15*15*15 cube      
    ...
    for i in range(len(wp)):
        ax1.scatter(wp[i][0], wp[i][1], 0, c='r', marker="$End$", s= 1000)

    ax1.set_xlim(-10, 80)
    ax1.set_ylim( 20,-70)
    ax1.set_zlim(0, 90) 

    ax1.view_init(azim=130, elev=20)
